...
import (
	...
	"github.com/brigadecore/brigade-github-gateway/internal/file"
...
func (s *server) ListenAndServe(ctx context.Context) error {
	...
	if s.config.TLSEnabled {
		...
		log.Printf(
			"Webhook receiver is listening with TLS enabled on 0.0.0.0:%d",
			s.config.Port,
		)
		...
		log.Printf(
			"Webhook receiver is listening without TLS on 0.0.0.0:%d",
			s.config.Port,
		)
